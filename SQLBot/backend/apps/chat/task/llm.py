...
{last_execute_sql_error}
    ...
    def generate_analysis(self, _session: Session):
        ...
        data_rows = data.get('data') if data else []
        self.chat_question.data = orjson.dumps(data_rows).decode()
        self.chat_question.data_profile = build_data_profile_text(data.get('fields') if data else [], data_rows)
    ...
    def check_save_sql(self, session: Session, res: str, operate: OperationEnum, prechecked: bool = False) -> str:
        sql, *_ = self.check_sql(session=session, res=res, operate=operate)
        if operate == OperationEnum.GENERATE_SQL_WITH_PERMISSIONS:
            sql, quoted = quote_known_table_identifiers(sql, self.table_name_list, self.ds.type)
            if quoted:
                SQLBotLogUtil.info("Permission SQL table identifiers auto-quoted before execution")
            self.validate_sql_tables(sql)
        elif operate != OperationEnum.GENERATE_DYNAMIC_SQL and not prechecked:
            sql = self.check_sql_before_execution(sql)
    ...
    def save_prepared_sql(self, session: Session, sql: str) -> str:
        save_sql(session=session, sql=sql, record_id=self.record.id)
        self.chat_question.sql = sql
        return sql
    ...
    def check_sql_before_execution(self, sql: str) -> str:
        try:
            checked_sql = check_sql_with_langchain(self.llm, sql, self.ds.type)
            if checked_sql != sql:
                SQLBotLogUtil.info("SQL query rewritten by LangChain query checker")
            checked_sql, quoted = quote_known_table_identifiers(checked_sql, self.table_name_list, self.ds.type)
            if quoted:
                SQLBotLogUtil.info("SQL table identifiers auto-quoted before execution")
            self.validate_sql_tables(checked_sql)
            return checked_sql
        except SingleMessageError:
            raise
        except Exception as e:
            SQLBotLogUtil.exception(f"LangChain SQL checker failed, using original SQL: {str(e)}")
            sql, quoted = quote_known_table_identifiers(sql, self.table_name_list, self.ds.type)
            if quoted:
                SQLBotLogUtil.info("SQL table identifiers auto-quoted before execution")
            self.validate_sql_tables(sql)
    ...
    def check_sql_and_prepare_execution(
            self,
            session: Session,
            res: str,
            operate: OperationEnum,
        ...
        sql, tables = self.check_sql(session=session, res=res, operate=operate)
        if operate != OperationEnum.GENERATE_DYNAMIC_SQL and not prechecked:
            sql = self.check_sql_before_execution(sql)
        else:
            self.validate_sql_tables(sql)
        actual_tables = extract_tables_from_sql(sql, ds_type=self.ds.type)
        return sql, sorted(actual_tables) if actual_tables else tables

    def repair_sql_after_execution_error(self, sql: str, error: Exception, schema: str = '') -> str:
        repaired_sql = repair_sql_with_langchain(self.llm, sql, self.ds.type, str(error), schema)
        repaired_sql, quoted = quote_known_table_identifiers(repaired_sql, self.table_name_list, self.ds.type)
        if quoted:
            SQLBotLogUtil.info("Repaired SQL table identifiers auto-quoted before retry")
        self.validate_sql_tables(repaired_sql)
        if repaired_sql == sql:
            raise error
        SQLBotLogUtil.info("SQL query repaired by LangChain after execution error")
        return repaired_sql
    ...
    def execute_sql_with_repair(
        ...
        # 查询结果缓存：首次尝试命中缓存
        ds_cache_id = self.ds.id if isinstance(self.ds, CoreDatasource) else None
        try:
            from apps.chat.cache.query_cache import get_query_cache, set_query_cache
            cached = get_query_cache(ds_cache_id, sql)
            if cached is not None:
                SQLBotLogUtil.info("SQL query cache hit, skip execution")
                return cached, sql
        except Exception as cache_err:
            SQLBotLogUtil.debug(f"Query cache lookup skipped: {cache_err}")

        current_sql = sql
        for attempt in range(max_retries + 1):
            try:
                result = self.execute_sql(sql=current_sql)
                # 执行成功后写入缓存
                try:
                    from apps.chat.cache.query_cache import set_query_cache as _set_cache
                    _set_cache(ds_cache_id, sql, result)
                except Exception as cache_err:
                    SQLBotLogUtil.debug(f"Query cache write skipped: {cache_err}")
                return result, current_sql
            except SQLBotDBError as e:
                if not allow_repair or attempt >= max_retries:
                    raise e
                SQLBotLogUtil.info(f"SQL execution failed (attempt {attempt + 1}/{max_retries}), attempting repair...")
                used_tables_schema, _ = self.out_ds_instance.get_db_schema(
                    self.ds.id, self.chat_question.question, embedding=False, table_list=tables
                ...
                current_sql = self.repair_sql_after_execution_error(current_sql, e, used_tables_schema)
                save_sql(session=session, sql=current_sql, record_id=self.record.id)
                self.chat_question.sql = current_sql
        # Should not reach here, but just in case
        raise SQLBotDBError("SQL execution failed after all retry attempts")

    def check_save_chart(self, session: Session, res: str) -> Dict[str, Any]:
        data = self.parse_chart_answer(res)
        profile = self.get_data_profile()

        try:
            if data.get('type') and data.get('type') != 'error':
                chart = enrich_chart_config(data, profile)
                self.normalize_chart_config_values(chart)
            elif data.get('type') == 'error':
                chart = self.build_fallback_chart(profile, data.get('reason'))
            else:
                chart = self.build_fallback_chart(profile, 'Chart config is empty')
        except Exception as e:
            SQLBotLogUtil.exception(f"Cannot normalize chart config, fallback to table: {str(e)}")
            chart = self.build_fallback_chart(profile, 'Cannot normalize chart config')

        save_chart(session=session, chart=orjson.dumps(chart).decode(), record_id=self.record.id)

        return chart

    def get_data_profile(self) -> Optional[Dict[str, Any]]:
        if not self.chat_question.data_profile:
            return None
        try:
            return orjson.loads(self.chat_question.data_profile)
        except Exception:
            return None

    def parse_chart_answer(self, res: str) -> Dict[str, Any]:
        json_str = extract_nested_json(res)
        if json_str is None:
            json_str = self.extract_relaxed_json(res)
        if json_str is None:
            SQLBotLogUtil.error("Cannot parse chart config from answer, fallback to table")
            return self.build_fallback_chart(self.get_data_profile(), 'Cannot parse chart config from answer')
        ...
        try:
            ...
            try:
                ...
                SQLBotLogUtil.error("Cannot parse chart config JSON after repair, fallback to table")
                return self.build_fallback_chart(self.get_data_profile(), 'Cannot parse chart config from answer')

        if not isinstance(data, dict):
            return self.build_fallback_chart(self.get_data_profile(), 'Chart config is not a JSON object')
        return data

    @staticmethod
    def extract_relaxed_json(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    def build_fallback_chart(self, profile: Optional[Dict[str, Any]], reason: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not fields and self.chat_question.fields:
            try:
                fields_obj = orjson.loads(self.chat_question.fields)
                if isinstance(fields_obj, list):
                    fields = [str(field) for field in fields_obj if field is not None]
            except Exception:
                fields = []
        columns = [{'name': field, 'value': field} for field in fields[:12]]
        summary = '图表配置解析不完整，已自动降级为表格展示当前查询结果。'
        if reason:
            summary += f' 原因：{reason}。'
        chart = {
            'type': 'table',
            ...
            'summary': summary,
            'columns': columns,
            'insights': ['当前结果以表格方式展示，可查看完整字段与明细数据。'],
            'alternatives': [],
        }
        if profile and profile.get('insights'):
            chart['insights'] = profile.get('insights')
        return chart

    @classmethod
    def normalize_chart_config_values(cls, chart: Dict[str, Any]):
        """Normalize all chart field references, including alternative charts."""
        cls.normalize_single_chart_config_values(chart)
        for alternative in chart.get('alternatives') or []:
            if isinstance(alternative, dict):
                cls.normalize_single_chart_config_values(alternative)

    @staticmethod
    def normalize_chart_field_value(value: str) -> str:
    ...
    @classmethod
    def normalize_axis_value(cls, obj: Dict[str, Any] | None):
        if isinstance(obj, dict) and obj.get('value') and isinstance(obj.get('value'), str):
            obj['value'] = cls.normalize_chart_field_value(obj.get('value'))

    @classmethod
    def normalize_single_chart_config_values(cls, chart: Dict[str, Any]):
        if chart.get('columns'):
            for v in chart.get('columns'):
                cls.normalize_axis_value(v)
        if chart.get('axis'):
            axis = chart.get('axis')
            if not isinstance(axis, dict):
                return
            if axis.get('x'):
                cls.normalize_axis_value(axis.get('x'))
            y_axis = axis.get('y')
            if y_axis:
                if isinstance(y_axis, list):
                    for item in y_axis:
                        cls.normalize_axis_value(item)
                elif isinstance(y_axis, dict):
                    cls.normalize_axis_value(y_axis)
            if axis.get('series'):
                cls.normalize_axis_value(axis.get('series'))
            if axis.get('multi-quota'):
                multi_quota = axis.get('multi-quota')
                if multi_quota.get('value'):
                    if isinstance(multi_quota['value'], list):
                        normalized_values = []
                        for v in multi_quota['value']:
                            if isinstance(v, str):
                                normalized_values.append(cls.normalize_chart_field_value(v))
                            else:
                                normalized_values.append(v)
                        multi_quota['value'] = normalized_values
                    elif isinstance(multi_quota['value'], str):
                        multi_quota['value'] = cls.normalize_chart_field_value(multi_quota['value'])
    ...
    def run_task(self, in_chat: bool = True, stream: bool = True,
        ...
        try:
            ...
            if not self.ds:
                if in_chat:
                    yield self.stream_progress(OperationEnum.CHOOSE_DATASOURCE)
                ...
                if in_chat:
                    ...
                    yield self.stream_progress(
                        OperationEnum.CHOOSE_DATASOURCE,
                        "success",
                        detail={"datasource": self.ds.name, "datasource_id": self.ds.id}
                    )
            ...
            # generate sql
            if in_chat:
                yield self.stream_progress(OperationEnum.GENERATE_SQL)
            ...
            if in_chat:
                ...
                yield self.stream_progress(OperationEnum.GENERATE_SQL, "success")
            # filter sql
            ...
            permission_sql_applied = False
            if in_chat:
                yield self.stream_progress(
                    OperationEnum.CHOOSE_TABLE,
                    message="Checking generated SQL before execution"
                )
            sql, tables = self.check_sql_and_prepare_execution(
                session=_session, res=full_sql_text, operate=sql_operate)
            if in_chat:
                yield self.stream_progress(
                    OperationEnum.CHOOSE_TABLE,
                    "success",
                    detail={"tables": tables or []}
                )
            ...
            if ((not self.current_assistant or is_page_embedded) and is_normal_user(
                ...
                if use_dynamic_ds:
                    if in_chat:
                        yield self.stream_progress(OperationEnum.GENERATE_DYNAMIC_SQL)
                    ...
                    if in_chat:
                        yield self.stream_progress(OperationEnum.GENERATE_SQL_WITH_PERMISSIONS)
                ...
                if sql_result:
                    ...
                    permission_sql_applied = True
                    tables = sorted(extract_tables_from_sql(sql, ds_type=self.ds.type))
                    if in_chat:
                        yield self.stream_progress(OperationEnum.GENERATE_SQL_WITH_PERMISSIONS, "success")
                    ...
                    assistant_dynamic_sql = self.check_save_sql(session=_session, res=sqlbot_temp_sql_text,
                                                                operate=sql_operate)
                    if in_chat:
                        yield self.stream_progress(OperationEnum.GENERATE_DYNAMIC_SQL, "success")
                else:
                    if in_chat:
                        yield self.stream_progress(
                            OperationEnum.GENERATE_DYNAMIC_SQL if use_dynamic_ds else OperationEnum.GENERATE_SQL_WITH_PERMISSIONS,
                            "skipped"
                        )
                    sql = self.save_prepared_sql(session=_session, sql=sql)
            else:
                sql = self.save_prepared_sql(session=_session, sql=sql)
            ...
            if in_chat:
                yield self.stream_progress(
                    ...
                    detail={"sql": sqlparse.format(real_execute_sql, reindent=True)}
                )
            allow_repair = not (permission_sql_applied or (sqlbot_temp_sql_text and assistant_dynamic_sql))
            result, real_execute_sql = self.execute_sql_with_repair(_session, real_execute_sql, tables, allow_repair)
            if real_execute_sql != sql:
                sql = real_execute_sql
                if not stream:
                    json_result['sql'] = sql
                if in_chat:
                    yield 'data:' + orjson.dumps(
                        {'content': sqlparse.format(sql, reindent=True), 'type': 'sql'}).decode() + '\n\n'
                    yield self.stream_progress(
                        OperationEnum.EXECUTE_SQL,
                        message="SQL repaired and retried",
                        detail={"sql": sqlparse.format(sql, reindent=True)}
                    )
            ...
            result["data"] = _data
            data_profile = build_data_profile(result.get('fields'), result.get('data'))
            self.chat_question.data_profile = build_data_profile_text(
                result.get('fields'), result.get('data'), data_profile)
            ...
            if in_chat:
                ...
                yield self.stream_progress(
                    OperationEnum.EXECUTE_SQL,
                    "success",
                    detail={"count": len(result.get('data') or [])}
                )
            if not stream:
            ...
            if in_chat:
                yield self.stream_progress(OperationEnum.GENERATE_CHART)
            ...
            if in_chat:
                ...
                yield self.stream_progress(OperationEnum.GENERATE_CHART, "success")
            ...
            if in_chat:
                ...
                yield self.stream_progress(
                    ...
                    "success",
                    detail={"chart_type": chart.get("type"), "title": chart.get("title")}
                )
            else:
                if stream:
                    # 图文并茂：先输出文字解读（summary），再输出数据表格/图表
                    summary_text = (chart.get('summary') or '').strip()
                    if summary_text:
                        # summary 自带分层 Markdown 结构，直接输出并加分隔线
                    ...
                    if chart.get('summary'):
                        json_result['summary'] = chart.get('summary')

            if in_chat:
                yield 'data:' + orjson.dumps({'type': 'finish'}).decode() + '\n\n'

                # 多维度分析（在 finish 之后异步生成，前端通过新事件类型接收）
                try:
                    from apps.chat.task.multi_dimension import MultiDimensionAnalyzer
                    analyzer = MultiDimensionAnalyzer()
                    dimensions = analyzer.suggest_dimensions(data_profile, self.chat_question.question)
                    if dimensions:
                        dimension_results = []
                        yield self.stream_progress(
                            OperationEnum.ANALYSIS,
                            detail={"count": len(dimensions)}
                        )
                        yield 'data:' + orjson.dumps({'type': 'dimension-start',
                                                      'count': len(dimensions)}).decode() + '\n\n'
                        for dim in dimensions:
                            dim_result = analyzer.generate_dimension_analysis(
                                self.llm, dim, data_profile,
                                self.chat_question.question,
                                self.chat_question.lang or '简体中文'
                            )
                            ...
                            yield self.stream_progress(
                                OperationEnum.ANALYSIS,
                                detail={"dimension": dim_result.get("dimension_title") or dim_result.get("dimension_type")}
                            )
                        if dimension_results:
                            result["dimensions"] = dimension_results
                            self.save_sql_data(session=_session, data_obj=result)
                        yield 'data:' + orjson.dumps({'type': 'dimension-finish'}).decode() + '\n\n'
                        yield self.stream_progress(OperationEnum.ANALYSIS, "success")
                except Exception as dim_e:
                    SQLBotLogUtil.exception(f"Multi-dimension analysis error: {dim_e}")
                    # 多维度分析失败不影响主流程
