...
function getContainerSize(container: S2MountContainer | null) {
  const parent = container instanceof HTMLElement ? container.parentElement : null
  const rect = (parent ?? (container as HTMLElement | null))?.getBoundingClientRect()
  return {
    width: Math.max(Math.floor(rect?.width || 600), 320),
    height: Math.max(Math.floor(rect?.height || 360), 240),
  }
}
...
export class Table extends BaseChart {
  ...
  init(axis: Array<ChartAxis>, data: Array<ChartData>) {
    ...
    const containerSize = getContainerSize(this.container)
