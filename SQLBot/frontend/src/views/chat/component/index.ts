import { BaseChart, type ChartTypes } from '@/views/chat/component/BaseChart.ts'
...
export function getChartInstance(type: string, id: string): BaseChart | undefined {
  const chartType = (CHART_TYPE_MAP[type] ? type : 'table') as ChartTypes
  if (isParent(CHART_TYPE_MAP[chartType], BaseChart)) {
    return new CHART_TYPE_MAP[chartType](id) as BaseChart
  }
  return undefined
}
